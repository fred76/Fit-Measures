...
extension PlicheMoethodController : AllertViewSkinFoldInsertDelegate {
	...
	func plicheGraph(){
		...
		case .jackson_3_Woman:
			if !UserDefaults.standard.bool(forKey: "fred76.com.ifit.skinFolds.unlock") {
				passDataToAlertButNotSave(customAlert: customAlert)
			} else {
				passDataToAlertAndSave(customAlert: customAlert)
			}
		case .jackson_3_Man:
			if !UserDefaults.standard.bool(forKey: "fred76.com.ifit.skinFolds.unlock") {
				passDataToAlertButNotSave(customAlert: customAlert)
	...
	func passDataToAlertButNotSave(customAlert : AllertViewSkinFoldInsertWithGraph){
		let (_, bodyDensity,bodyFatPerc,leanMass,fatMass, lastWeight) = pippo.shared.plicheAddedByUser(abdominal: abdominal, biceps: biceps, chest: chest,  midaxillary: midaxillary, subscapular: subscapular, suprailiac: suprailiac, thigh: thigh, triceps: triceps, viewVontroller: self, plicheMethod: plicheMethod.rawValue)
	...
	func passDataToAlertAndSave(customAlert : AllertViewSkinFoldInsertWithGraph){
